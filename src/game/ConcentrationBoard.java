...
public class ConcentrationBoard {
    ...
    public ConcentrationCard getCard(int row, int col) throws ConcentrationException {
        //TODO YOUR CODE HERE
        if(row> this.DIM-1|| col>this.DIM-1){
            throw new ConcentrationException("ERROR Invalid Card position entered");
        }else {
    ...
    public CardMatch reveal(int row, int col) throws ConcentrationException {
        //TODO YOUR CODE HERE
        if(this.gameOver()){
            throw new ConcentrationException("GAME_OVER No further input required");
        }else if((row> this.DIM-1|| col>this.DIM-1)){
            throw new ConcentrationException("ERROR Invalid Card position entered");
        } else if(!this.board[row][col].isHidden()){
            throw new ConcentrationException("ERROR Card already revealed");
        }
        if(this.revealedCard==null){
            this.revealedCard = this.board[row][col];
            this.revealedCard.reveal();
            return null;
        }
        ConcentrationCard cardOne = this.revealedCard;
        this.revealedCard = null;
        ConcentrationCard cardTwo = this.board[row][col];
        cardTwo.reveal();
        boolean match = (cardOne.getLetter() == cardTwo.getLetter());
        if(match){
            this.matches+=1;
        }
        return new CardMatch(cardOne,cardTwo,match);
    }
    ...
    public boolean gameOver() {
        //TODO YOUR CODE HERE
        return this.matches==((this.getDIM()*this.getDIM())/2);
    }
